...
export default class Paddle {
  ...
  update(dt, game) {
    ...
    if (this.type === "player1" || this.type === "player2") {
      ...
      const aiMultiplier = (game && game.difficulty) ? game.difficulty.getAILevel(game.scoreBoard.scores.player1 || 0) : 1.0;
      const paddleBaseSpeed = (game && game.difficulty) ? game.difficulty.getPaddleSpeed() : this.speed;
      const maxMove = paddleBaseSpeed * (1 + (aiMultiplier - 1)) * dt * 60 * 1.1; // scale
      const targetY = ball.y - this.height / 2;
      const diff = targetY - this.y;
      // Move towards target but clamp by maxMove to avoid teleporting

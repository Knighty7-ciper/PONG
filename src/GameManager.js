...
export default class GameManager {
  ...
  _keyHandler(e) {
    ...
    if (e.key === 'm' || e.key === 'M') { this.audio.toggleMute(); this.saveSettings(); }
    if (e.key === 'Tab') { this.toggleMultiplayer(); e.preventDefault(); }
    if (e.key === 'c' || e.key === 'C') { this.cycleCustomization(); this.saveSettings(); }
    if (e.key === 'p' || e.key === 'P' || e.key === 'Escape') this.togglePause();
    if (e.key === 'o' || e.key === 'O') this.settings.toggle();
  ...
  setupEntities() {
    ...
    const ballSpeed = (this.difficulty && typeof this.difficulty.getBallSpeed === 'function') ? this.difficulty.getBallSpeed() : 6;
    const paddleSpeed = (this.difficulty && typeof this.difficulty.getPaddleSpeed === 'function') ? this.difficulty.getPaddleSpeed() : 6;
    ...
    this.players = [
      ...
      new Paddle(this.canvas.width - 32, this.canvas.height / 2 - 48, 16, 96, "#ff2d55", this.multiplayer ? 'player2' : 'ai')
    ];

    // apply paddle speed from difficulty
    this.players.forEach(p => { p.speed = paddleSpeed; });

    this.balls = [ new Ball(this.canvas.width / 2, this.canvas.height / 2, 13, ballSkin.color, ballSpeed) ];
    this.powerUps = [];
    this.obstacles = [
      new Obstacle(this.canvas.width/2-22, this.canvas.height/2-60, 44, 44, "block",
        (t, bx, by) => ({ y: by + Math.sin(t/300)*110 }))
    ];
  ...
  update(dt, ts) {
    ...
    if (Math.random() < dt * 0.12 && this.powerUps.length < 2) {
      let types = ["enlarge", "shrink", "multi", "slow", "speed", "invisible"];
      let px = Math.random() * (this.canvas.width - 120) + 60;
      let py = Math.random() * (this.canvas.height - 120) + 60;
    ...
    for (const ball of this.balls) {
      for (const ob of this.obstacles) {
        if (ob.checkCollision(ball)) {
          ...
          ball.velocityX *= -1; ball.velocityY *= -1;
  ...
  draw(ts) {
    ...
    if (typeof this.drawBackground === 'function') {
      try { this.drawBackground(); } catch (e) { console.warn('drawBackground() failed', e); this._drawSimpleBackground(); }
    } else { this._drawSimpleBackground(); }

    if (typeof this.drawNet === 'function') {
      try { this.drawNet(); } catch (e) { console.warn('drawNet() failed', e); this._drawSimpleNet(); }
  ...
  onScore(winner) {
    ...
    if (lastFrames.length > 0) {
      try { this.replay.saveLast(lastFrames); } catch {}
    ...
    const p2 = this.scoreBoard.scores.ai || (this.scoreBoard.scores.player2 || 0);
    if (p1 >= ws || p2 >= ws) {
      try { this.highScores.save(this.scoreBoard.mode, p1); } catch {}
      this.endGame();
    } else {
      this.resetPositions();
    }
  ...
  endGame() {
    ...
    this.showOverlay(
      title,
      `Final Score:<br><b>You</b>: ${p1} &nbsp;&nbsp; <b>AI</b>: ${p2}<br><br>
       ${replayBtn} ${bestBtn}
    ...
    setTimeout(() => {
      ...
      if (lastBtn) lastBtn.onclick = () => { const f = this.replay.getLast(); if (f) { this.replay.startReplayFromFrames(f); } };
      const bestBtnEl = document.getElementById('play-best-rally');
      if (bestBtnEl) bestBtnEl.onclick = () => { const f = this.replay.getBest(); if (f) { this.replay.startReplayFromFrames(f); } };
    }, 100);
  }
  ...
  drawReplayFrame(state) {
    this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
    if (typeof this.drawBackground === 'function') {
      try { this.drawBackground(); } catch (e) { this._drawSimpleBackground(); }
    } else this._drawSimpleBackground();

    if (typeof this.drawNet === 'function') {
      try { this.drawNet(); } catch (e) { this._drawSimpleNet(); }
    } else this._drawSimpleNet();
  ...
  toggleMultiplayer() {
    ...
    try { if (this.online && typeof this.online.stop === 'function') this.online.stop(); } catch (e) {}
    this.setupEntities();
    // recreate input manager so it binds to updated players
    try { if (this.input && typeof this.input.stop === 'function') this.input.stop(); } catch (e) {}
    this.input = new InputManager(this);
    this.saveSettings();
  }
  ...
  startGame() {
    ...
    this.setupEntities();
    try { if (!this.audio.muted) this.audio.playMusic(this.theme.getCurrent().music); } catch (e) {}
    this.saveSettings();
  }

  destroy() {
    try { if (this.online && typeof this.online.stop === 'function') this.online.stop(); } catch (e) {}
    try { if (this.gamepad && typeof this.gamepad.stop === 'function') this.gamepad.stop(); } catch (e) {}
    try {
      document.removeEventListener('keydown', this._boundKeyHandler);
      this.canvas.removeEventListener('dblclick', this._boundDblClick);
    } catch (e) {}
    try { if (this.input && typeof this.input.stop === 'function') this.input.stop(); } catch (e) {}
    try { this.replay.reset(); } catch (e) {}

...
export default class InputManager {
  ...
  _onKeyDown(e) {
    ...
    if (e.key === 'w' || e.key === 'W' || e.key === 'ArrowUp') {
      ...
      p2.moveUp = true; p2.moveDown = false;
    } else if ((e.key === 'ArrowRight' || e.key === 'd' || e.key === 'D') && p2) {
      p2.moveDown = true; p2.moveUp = false;
    }
  }
  ...
  _onKeyUp(e) {
    ...
    const p2 = this.game.players[1];
    if (!p1) return;
    if (e.key === 'w' || e.key === 'W' || e.key === 'ArrowUp') p1.moveUp = false;
    if (e.key === 's' || e.key === 'S' || e.key === 'ArrowDown') p1.moveDown = false;

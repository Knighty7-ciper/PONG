  ...
  update(dt, game) {
    ...
    for (const paddle of game.players) {
      if (
        ...
        let impact = ((this.y - (paddle.y + paddle.height / 2)) / (paddle.height / 2));
        let angle = (Math.PI / 4) * impact;
        let dir = paddle.x < game.canvas.width / 2 ? 1 : -1;
        ...
        window.dispatchEvent(new CustomEvent('pong-log', { detail: { text: `Paddle hit at y=${Math.round(this.y)} by ${paddle.type}`, category: 'hit' } }));
    ...
    if (this.x - this.radius < 0) {
      window.dispatchEvent(new CustomEvent('pong-log', { detail: { text: 'Ball out left — AI scores', category: 'score' } }));
      if (game && typeof game.onScore === 'function') game.onScore('ai');
      return;
    }
    if (this.x + this.radius > game.canvas.width) {
      window.dispatchEvent(new CustomEvent('pong-log', { detail: { text: 'Ball out right — Player scores', category: 'score' } }));

...
export default class OnlineMultiplayer {
  ...
  stop() {
    ...
    this.dataChannel = null; this.pc = null; this.ws = null; this.connected = false;
  ...
  _setupDataChannel() {
    ...
    this.dataChannel.onopen = () => { this._log('Data channel open'); };
    this.dataChannel.onclose = () => { this._log('Data channel closed'); };
  ...
  sendInput(input) {
    ...
    if (this.dataChannel && this.dataChannel.readyState === 'open') {
      try { this.dataChannel.send(JSON.stringify({ type: 'input', input })); } catch (e) {}
  ...
  async _connectSignaling() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.onopen = () => { this._log('Connected to signaling server'); resolve(); };
      this.ws.onerror = (e) => { console.warn('Signaling error', e); reject(e); };
      this.ws.onmessage = async (ev) => {
        let data;
        try { data = JSON.parse(ev.data); } catch { return; }
        await this._handleSignalingMessage(data);
      };
      this.ws.onclose = () => { this._log('Signaling closed'); };
    });
  }
  ...
  _log(...args) {
    ...
    window.dispatchEvent(new CustomEvent('pong-log', { detail: { text: '[OnlineMultiplayer] ' + args.join(' '), category: 'info' } }));
  }
}